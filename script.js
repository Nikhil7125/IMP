...
class Paper {
  ...
  init(paper) {
    paper.addEventListener('touchmove', (e) => {
      ...
      const velX = touch.clientX - this.touchX;
      const velY = touch.clientY - this.touchY;

      if (!this.rotating && this.holdingPaper && this.isDragging(velX, velY)) {
        this.currentPaperX += velX;
        this.currentPaperY += velY;
  ...
  isDragging(velX, velY) {
    const threshold = 5; // Adjust as needed
    return Math.abs(velX) > threshold || Math.abs(velY) > threshold;
  }

  isClick() {
    const threshold = 5; // Adjust as needed
    return Math.abs(this.touchX - this.startTouchX) < threshold &&
           Math.abs(this.touchY - this.startTouchY) < threshold;
  }
}
